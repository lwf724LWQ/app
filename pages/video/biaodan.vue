          ...
          <view class="option-header-row inline-input">
            ...
            <view v-if="option.hasInputs" class="inline-input-box flex-1">
              ...
              <view class="grid-container flex-1">
                <input
                  class="item"
                  v-for="(item, inputIndex) in option.inputCount"
                  :key="inputIndex"
                  v-model="option.inputs[inputIndex]"
                  placeholder=""
                />
              </view>
              <!-- 5个输入框 -->
...
import { apiWordInsert, apiGetIssueNo } from "@/api/apis.js";
...
onLoad(async (pageOptions) => {
  ...
  const issueInfo = await getCurrentIssueInfo(decodeURIComponent(pageOptions.tname || ""));

  formData.value = {
    videoId: decodeURIComponent(pageOptions.videoId || ""),
    tname: decodeURIComponent(pageOptions.tname || ""),
    name1: userStore.getUserInfo.nickname,
    issueNo: issueInfo.issueno,
    date: issueInfo.opendate,
  };

  const type = ["福彩3D", "排列3", "排列三"].includes(formData.value.tname) ? 0 : 1;
...
// 获取当前彩票类型期号和开奖时间
async function getCurrentIssueInfo(tname) {
  const res = await apiGetIssueNo({
    tname: tname,
  });
  return res.data;
}
...
function base64ToBlob(base64String, mimeType = "image/png") {
  ...
  const date = moment(formData.value.date);
  // 返回Blob对象
  return new File([new Blob([array], { type: mimeType })], `form-${new Date().getTime()}.png`, {
    type: mimeType,
  });
}
...
const handleSubmit = async () => {
  ...
  const videoId = formData.value.videoId;
  const issueno = formData.value.issueNo;
  const tname = formData.value.tname;
  const opendate = formData.value.date;
